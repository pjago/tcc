(ns pjago.repl ;copy of arcadia.repl
  (:refer-clojure :exclude [with-bindings])
  (:require [clojure.main :as main]
            [clojure.string :as str]
            ; [arcadia.config :as config] ; it doesn't load on build @pjago
  ...
  (with-out-str
    (binding [*err* *out*]
      ((if *pprint-output* pprint/pprint prn)
       (let [value (eval
                     `(do
                        ~(when-let [inj (read-string* "(use 'clojure.repl)")]
                                        ; (read-string*
...
(defn start-server [^long port]
  (let [editor-running 
        (if-let [ar (find-ns 'arcadia.repl)]
          @(ns-resolve ar 'server-running)
          false)]
    (if (or @server-running editor-running)
      (Debug/Log "REPL already running")
      (Debug/Log "Starting REPL"))
    (when-not (or @server-running editor-running) 
      ...
      (let [socket (UdpClient. (IPEndPoint. IPAddress/Any port))]
        ...
        (.Start (Thread. (gen-delegate ThreadStart []
                                       (if false ;((config/config) :verbose)
                                         (Debug/Log "Starting REPL..."))
