...
uint8_t ServoTimer1::attach(int pinArg, uint16_t min, uint16_t max)
{
	if ( pinArg != 11 && pinArg != 12) return 0;
	pin = pinArg;
	angle = NO_ANGLE;
...
void ServoTimer1::writeMicroseconds(uint16_t value)
{
//	uint16_t min = min16*16;
//	uint16_t max = min16*16;
//	if (value > max)
//		value = max;
//	else if (value < min)
//		value = min;

	if ( pin == 11) OCR1A = value*clockCyclesPerMicrosecond()/8L;
    else OCR1B = value*clockCyclesPerMicrosecond()/8L;
}
...
{
    if ( pin == 11 && attached9) return 1;
    if ( pin == 12 && attached10) return 1;
    return 0;
}
