...
(defn- type? [x] ;perhaps I have to change this to Type
  (instance? System.Type x))

(defn- same-or-subclass? [^System.Type t1, ^System.Type t2]
  (or (= t1 t2)
    (.IsSubclassOf t2 t1)))
...
(defn- ensure-type ^System.Type [t]
  (cond
    (symbol? t) (resolve t)
    (type? t) t
    :else (throw
...
(defn- type-symbol [^System.Type t]
  (symbol (.FullName t)))
...
(defn- keyword-for-type [t]
  (let [^System.Type t (ensure-type t)]
    (nice-keyword
      (.Name t))))
...
(defn- extract-property ;; this is stupid
  ...
  (let [^System.Type t (resolve declaring-class)]
    (->> (.GetProperties t)
      (filter
...
(defn- all-component-types ;had to specify the assemblies here, @pjago
  ([] (all-component-types [(load-assembly "UnityEngine.CoreModule") (load-assembly "UnityEngine.PhysicsModule")]))
  ([assems]
     (for [^Assembly assem assems
           ^System.Type t (.GetTypes assem)
           :when (and
...
(defn- all-value-types
  ([] (all-value-types [(load-assembly "UnityEngine.CoreModule") (load-assembly "UnityEngine.PhysicsModule")]))
  ([assems]
     (for [^Assembly assem assems
           ^System.Type t (.GetTypes assem)
           :when (and
...
(defn- populater-form
  ...
  ([typesym ctx]
     (if (let [^System.Type t (resolve typesym)] ;this is genuinely stupid
           (.IsValueType t))
...
(defn- constructors-spec [type-or-typesym]
  (let [^System.Type t (ensure-type type-or-typesym)]
    (set
      (conj
        (for [^System.Reflection.ConstructorInfo cinf (.GetConstructors t)
...
(defn- game-object-hydrate-case-default-fn [ctx]
  (mu/checked-keys [[targsym keysym valsym] ctx]
    `(do
       (if-let [^System.Type t# (resolve-type-flag ~keysym)]
