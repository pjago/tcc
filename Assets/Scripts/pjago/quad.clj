...
(def vmax 6.761932205804940) ;maximum emf
(def rho 1.2754) ;[kg:m³] CNPT air density
(def kv (/ 1000 m/rpm.s:rad)) ;1000 kv | broke! 930 kv motors
(def ke (/ kv)) ;in theory should be equal to kt?
(def eta 0.75) ;that's in the A2212 datasheet (minimum efficiency)
(def ra (* 2 0.09)) ;because 90 mOhms is for one coil
(def la (* 0.125e-3)) ;this is in some datasheet I believe?
(def wmax (* kv vmax))
(def u->w #(* wmax (m/sqrt (max 0 %))))
(def w->u #(m/pow (/ % wmax) 2))
      ...
      (with-cmpt p [ptr Transform]
        (as/let [r (.MultiplyPoint WL (. ptr position))
          ...
          (if-let [fifo (state p :spin/fifo)]
            ...
            ;     (state+ p :spin (u->w u-1))))
            ;4th Attempt: (this is right but the frequency maybe some 1st order filter)
            ;The point I measured has to use an small AC approximation
            ;The bias was 20%, corresponding to a inertia jrot
            ;now I only need to oppose the actual drag
            ;things are much clearer now, but the gain problem still exists
            ;if the setpoints were reached, but the torque stops at half of it
            (let [ctrl (state p :ctrl/mech)
                  aero (state p :aero)
                  uset (state p :spin/set)
                  wset (u->w uset)
                  ur (+ (* ke wset) (/ (m/abs (.Drag aero wset 0.0)) (/ ke ra) eta))
                  ur-1 (first fifo)
                  _ (update-state p :spin/fifo conj ur)
                  ur-0 (first (state p :spin/fifo))
                  ur (if (> (rand) 0.5) ur-0 ur-1)
                  uy (* ke (state p :spin))
                  y (m/abs (/ (state p :drag) eta))           ;* 2
                  u (step-next ctrl (* 1 (/ ke ra) (- ur uy)) (* 1 y))]
              (state+ p :spin u))   ;* 2
...
(defmutable Martins3 ;? 10x4.5
  ...
  (Ve [dyn w] ;air exit velocity
    (* w pitch m/m:rad.in))
  (Ct [_ j] ;thurst coeficient
    0.085227078507371) ;0.088525576065668) ;0.128627856016643)
  (Cp [_ j] ;drag coeficient
    0.034334991341376) ;0.035574334354053) ;0.051689585773344
  (Drag [dyn w v] ; is this wrong ???
    (* 2.5721714368E-10 rho (m/pow w 2)
       (/ m/tau) ;whyyyyyyyyy
       (m/pow diameter 5)
       (.Cp dyn 0)
       (if clockwise 1 -1)))
...
(defmutable Martins0 ;? 10x4.5
  ...
  (Ve [dyn w] ;air exit velocity
    (* w pitch m/m:rad.in))
  (Ct [_ j] ;thurst coeficient
    0.059381666452880) ;0.086281691483864) ;0.075239066087604
  (Cp [_ j] ;drag coeficient
    0.023862745104630)
  (Drag [dyn w v]
    (* 2.5721714368E-10 rho (m/pow w 2)
...
(defctrl KICtrl {:tune set-ki :u uki})
...
(defn new-pid
  ([m ctor] (apply new-pid m/nan m/nan m/nan 1 ctor (reduce into m)))
...
(defn reset [ctrl]
  (let [buffers [(.r ctrl) (.e ctrl) (.e ctrl) (.u ctrl)]
    ...
    (if (instance? PI_PDCtrl ctrl) ;bad sign @todo
      (set! (.u1-1 ctrl) 0.0))
    (set! (.u ctrl) uo))
  ctrl)
...
(defn new-relay
  ([amp eps] (new-relay amp (- amp) eps 1 map->RelayCtrl))
...
(def dmax (.Drag pm3 (u->w 1) 0.0))
(def jrot ;@tcc
  (-> (.Drag pm3 (u->w 0.25) 0.0)
      (/ 0.5 (u->w 0.25) eta)
      (+ (* ke ke (/ ra)))
      (/ 9.207760292564952)))
