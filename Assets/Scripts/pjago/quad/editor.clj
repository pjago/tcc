(do (in-ns 'pjago.quad)
    (import '[UnityEngine GameObject Application Input KeyCode Canvas Screen GUI Color LineRenderer Rect ParticleSystem TrailRenderer]
...
(defn aero-info [^GameObject gob k]
  ; (when (= (count (objects-tagged (str ::quad))) 1))
    (let [{:keys [thrust drag spin speed]} (state gob)
      ...
      (label [0 15 W H] (format "T: %.2f" (float (/ drag dmax)))) ;N.m
    ...
    (let [{:keys [props]} (state gob)
      ...
      (dotimes [i (count props)]
        (as/let [p (nth props i) {:keys [thrust drag]} (state p)]
          ...
          (label [W (* i 15) W H]
            (if (neg? drag)
              (format "T(%d):  %+.4f" i (float (/ drag dmax)))
              (format "T(%d): %+.4f" i (float (/ drag dmax)))))))))
...
(def default-canvas
  (merge
    ^{::x/init ::canvas} {:time {:state 0} :watches {:state #{::quad ::propeller}}}
...
(defn canvas-spit-watch 
  ...
  ([gob]
   (let [togg (select-keys (state gob) (keys (state gob :toggles)))
         info {:n 1 :dt Time/fixedDeltaTime :canvas-state togg}]
     (into []
...
(defn canvas-pause [gob]
  (let [watches (into [] cw (state gob :watches))]
    (doseq [fxup (keep #(cmpt % (:fixed-update hook-types)) watches)]
     ...
     (if (obj-nil q)
       (let [{:ctrl/keys [height yaw pitch roll euler]} (state q)
         ...
         (doseq [[p o] (partition 2 (interleave (state q :props) (state q :ctrl/offset)))]
           ; (update-state p :ctrl/mech reset)
...
(defmethod canvas-toggle :relay [gob k]
  (when-let [q (get-player gob)]
    (if-let [cc (state q :canvas/ctrl)]
      (let [{:ctrl/keys [height yaw roll pitch]} cc]
...
(def qixx (float 0.019983947332022)) ;@tcc
(def qangular-drag (float 0.899399399399399)) ;@tcc
(def hj-height (v3 0.0 (/ 0.181961172815234 9.81) 0.0)) ;@tcc
(def moff (v3 0.0 -0.015 0.0))

(defn pinext [^GameObject gob k]
  (if-cmpt gob [lr LineRenderer]
    (let [points (new |UnityEngine.Vector3[]| 3)
          [start end] (state gob k)]
      (if (and (obj-nil start) (obj-nil end))
        (do (aset points 0 (v3+ (.. start transform position) moff))
            (aset points 1 (.. gob transform position))
            (aset points 2 (v3+ (.. end transform position) moff))
            (.SetPositions lr points))
        (cmpt- gob LineRenderer)))))

(defn pinpoint ;todo: use the same color as the battery
  ([] [((x/paint ::M2) (x/wrap :pinpoint))])
  ([gob]
   (let [mr (cmpt gob MeshRenderer)
         lr (cmpt+ gob LineRenderer)]
     (set! (.sharedMaterial lr) (.sharedMaterial mr))
     (set! (.widthMultiplier lr) (float 0.004))
     (set! (.positionCount lr) (short 3))
     (set! (.generateLightingData lr) true)
     (set! (.useWorldSpace lr) true))
   (set! (.isTrigger (cmpt+ gob SphereCollider)) true)
   (set! (.. gob transform position) hj-height)
   (set! (.isKinematic (cmpt+ gob Rigidbody)) true)))
...
(defmethod canvas-toggle ::constraint [gob k]
  ...
  (if-let [q (get-player gob)]
    (let [{:keys [height roll pitch yaw]} (state gob)
      ...
      (destroy-immediate (state q :pinpoint))
      (compute-tensor q)
      ...
      (doseq [ck (keys default-ctrl) :let [k (keyword (name ck))]]
        ...
        (when-let [ctrl (get (state q :canvas/ctrl) ck)]
          (set! (.enabled ctrl) (state gob k))))
      (run! #(set! (.enabled %) true) lca)
      (cond
        (= mask 4)
        (with-cmpt q [hj HingeJoint]
          (let [[p0 _ p2 _] (state q :props)
                pin (first (render pinpoint))
                prb (cmpt pin Rigidbody)]
            (hook+ pin :update :fixed #'pinext)
            (state+ pin :fixed [p0 p2])
            (state+ q :pinpoint pin)
            (.. pin transform (Translate (.. q transform position) Space/World))
            (set! (.connectedBody hj) prb)
            (set! (.axis hj) forward)
            (run! #(do (set! (.enabled %) false)
                       (follow-pos (gobj %) :follow)) ;otherwise it misses 1
                  lca)
            (set! (.anchor hj) 
              (-> q .transform .worldToLocalMatrix
                  (.MultiplyPoint (.. pin transform position))
                  (v3+ (v3map / (.centerOfMass rb) (.. q transform lossyScale)))))
            (set! (.autoConfigureConnectedAnchor hj) false)
            (state+ q :ctrl/offset [0.0 0.25 0.0 0.25])))
        (= mask 2)
        (with-cmpt q [hj HingeJoint]
          (let [[_ p1 _ p3] (state q :props)
                pin (first (render pinpoint))
                prb (cmpt pin Rigidbody)]
            (hook+ pin :update :fixed #'pinext)
            (state+ pin :fixed [p1 p3])
            (state+ q :pinpoint pin)
            (.. pin transform (Translate (.. q transform position) Space/World))
            (set! (.connectedBody hj) prb)
            (set! (.axis hj) right)
            ...
            (set! (.anchor hj) 
              (-> q .transform .worldToLocalMatrix
                  ...
                  (v3+ (v3map / (.centerOfMass rb) (.. q transform lossyScale)))))
            (set! (.autoConfigureConnectedAnchor hj) false)
            (state+ q :ctrl/offset [0.25 0.0 0.25 0.0])))
        (= mask 0)
        (do (set! (.constraints rb) RigidbodyConstraints/FreezeAll)
            (state+ q :ctrl/offset [0.0 0.0 0.0 0.0]))
        :else
...
(defn canvas-update [gob _] ;probably the worst code I've ever made. nice!
  (let [bar (state gob :scrollbar)
        ...
        now Time/time
        cam (first (state gob :camera))
        pxyz (state gob :mouse/previous) ;starts with no default?? @bug
        xyz (.ScreenToWorldPoint (cmpt cam Camera) Input/mousePosition)
        ray (.ScreenPointToRay (cmpt cam Camera) Input/mousePosition)
        whe (* 4 (Input/GetAxis "Mouse ScrollWheel"))
        hit (UnityEngine.RaycastHit.)] ;couldn't put by-ref to work
        ;shouldn't move when the object is after some UI @bug
    (when-let [g (and (Input/GetKey KeyCode/Mouse0) (state gob :mouse/drag))]
      (let [rb (cmpt g Rigidbody)
            in (-> (Quaternion/Inverse (.. cam transform rotation))
                   (qv* (v3- xyz pxyz))
                   (v3+ (v3 0 0 whe)))]
        (if (and (obj-nil rb) (not (.isKinematic rb)))
          (.AddForceAtPosition rb (Vector3/Normalize in) (.point (state gob :mouse/grab)))
          (.Translate (.transform g) in Space/World)))
      (if (or (Input/GetKey KeyCode/LeftControl) (Input/GetKey KeyCode/RightControl))
        (if-cmpt (obj-nil (state gob :player)) [rb Rigidbody]
          (if-let [hj (cmpt rb HingeJoint)]
            (when-let [pin (obj-nil (state rb :pinpoint))]
              (set! (.. pin transform position) 
                    (v3 (.. rb transform position x) 
                        (.. pin transform position y)
                        (.. rb transform position z)))
              (set! (.anchor hj)
                (v3map /
                       (v3+ (v3 0.0 (- (.. pin transform position y) (.. rb transform position y)) 0.0)
                            (.centerOfMass rb))
                       (.. rb transform lossyScale))))))))
    (state+ gob :mouse/previous xyz)
    (when (Input/GetKeyDown KeyCode/Mouse0)
      (when-let [hit (first (seq (Physics/RaycastAll ray)))]
        (state+ gob :mouse/drag (.. hit collider gameObject transform root))
        (state+ gob :mouse/grab hit)))
    (when (Input/GetKeyUp KeyCode/Mouse0)
      (state+ gob :mouse/drag nil))
    (when-let [q (obj-nil (state gob :player))]
      ...
      (when (Input/GetKeyDown KeyCode/Mouse0)
        (if-let [lock-pos (state cam :follow)]
          (set! (.lock lock-pos) nil)))
      (when (Input/GetKeyUp KeyCode/Mouse0)
        (if-let [lock-pos (state cam :follow)]
          (set! (.lock lock-pos) q))))
        ...
        (do (if-let [q (obj-nil (state gob :player))]
            ...
            (let [k (state gob :scrollbar/key)
              ...
              (if (= (namespace k) "player")
                (set! (.text v) (format "%.4f" (state gob k))))
    ...
    (if (Input/GetKeyDown KeyCode/Delete)
      (when-let [drag (state gob :mouse/drag)]
        (state+ gob :mouse/drag nil)
        (destroy (gobj drag))))
    (if (and (Input/GetKeyDown KeyCode/Tab) (not (Input/GetKeyDown KeyCode/LeftAlt)))
      (if esc
        (let [rev (Input/GetKey KeyCode/LeftShift)
        ...
        (if (or (Input/GetKey KeyCode/LeftShift) (Input/GetKey KeyCode/RightShift))
          (let [rev (Input/GetKey KeyCode/LeftShift)
                all (vec (cond-> (objects-tagged (str ::quad)) rev (-> vec rseq)))
                pre (get-player gob)
...
(defn canvas-scene
  ...
  ([graph]
   (let [{:keys [::canvas :last-camera :other-camera ::x/scene]} 
     ...
     (state+ canvas :camera [other-camera last-camera])
     (.SetActive wind false)
     (.SetActive last-camera false))))
  ...
  ([q]
   (when-let [c (object-tagged (str ::canvas))]
     (let [toggles (keys (state c :toggles))
       ...
       (state+ q :canvas/toggle (select-keys (state c) toggles))
       (state+ q :canvas/ctrl (select-keys (state q) ctrls)))
     (switch-player c q)
     (with-cmpt q [rb Rigidbody] 
       (set! (.angularDrag rb) qangular-drag))
     (state+ q :inertia-tensor (v3 qixx (* 2 qixx) qixx))
     (state+ q :center-of-mass (v3 0.0)) ;keep it simple
     (compute-tensor q)
     (canvas-restart c q))))
...
(defn relay-3
  ([] (relay-3 {}))
  ([{:keys [no-hinge space height yaw pitch roll]
      :or {no-hinge false space 1 height [0.01 0.1] yaw [0.2 1] pitch [0.02 1] roll [0.02 1]}}]
   (switch-player nil)
   (when-let [c (object-tagged (str ::canvas))]
     (let [qmid (volatile! nil)]
       ...
       (when pitch
         ...
         (let [x (- space) z (- x) peps (map * [0 1 2] (repeat (second pitch)))]
           (doseq [[y eps] (map list [-0.5 0.0 0.5] peps)
             ...
             (if-let [hj (cmpt q HingeJoint)]
               (let [ca (.connectedAnchor hj)]
                 (set! (.. hj connectedBody transform position) 
                       (v3+ ca (v3 x y z)))))
       ...
       (when roll
         (state+ c :yaw false)
         (state+ c :roll true)
         (if no-hinge (state+ c :pitch true))
