...
(defn full-watch [gob & _]
  ...
  (state+ gob :aero (mapv #(state % :aero) (state gob :props)))
  (state+ gob :ctrl/props (mapv #(state % :ctrl/mech) (state gob :props)))
  (reduce-kv watch+ gob
    #:ctrl
...
(defn compute-tensor [^GameObject gob & _] ;@tcc
  (let [rb (ensure-cmpt gob Rigidbody)]
    (if-let [m (float (state gob :self-mass))]
      (set! (.mass rb) m))
    (if-let [c (state gob :center-of-mass)]
      (set! (.centerOfMass rb) c)
      (set! (.centerOfMass rb) (center-of-mass gob)))
    (set! (.inertiaTensorRotation rb) (qt)) ;todo: diagonalize
    (if-let [i (state gob :inertia-tensor)]
      (set! (.inertiaTensor rb) i)
  ...
  ([gob]
   (let [child-seq (rest (gobj-seq gob))
     ...
     (dotimes [i (count propeller)]
       (let [p (nth propeller i) ;0.114
             m (new-pid {:ki (/ 1 jrot)} map->KICtrl) ;@hard
             ;e (new-pid {:z (/ ra la)})
             a (state p :aero)
             a ((nth prop4 i)
...
;this is so buggy @bug @bug @bug
(defn qgen []
  (let [foo (GameObject. "foo")]
    (comp #(x/doto %
             (fn [gob]
               (hook+ foo :on-destroy (fn [& _] (full-watch gob)))
               (destroy foo 0)))
...
(def zn (partial new-pid 0.001416271324645 m/inf 0.272943881395627 1 map->PIDCtrl))

;[kp, ti, td] = pplace(Ga, [1 1.75 2.15 1].*[1 4 4^2 4^3], 'I+PD')
;C = pid2(kp, kp/ti, kp*td, 0.0, 0.0, 0.0)
;[C1, C2] = getComponents(C, 'feedback')
(def i-pd (partial new-pid 0.006700791674117 0.5375 0.177348837209302 1 map->I_PDCtrl))

;[C1, C2] = ctrl(Gs, 'CMP+D', [1 2.1 3.4 2.7 1].*[1 8 8^2 8^3 8^4])
(def lead-d (partial new-comp-d 0.008339532231879 0.8055 0.8747 0.002995543501783 1))
  ...
  (let [x (lead-d) y (lead-dy) z (lead-d)]
    (merge qon
      ...
      {:pitch {:state x}
       :yaw {:state y}
       :roll {:state z}
       :euler {:state (new-euler x y z map->Euler2Ctrl)
